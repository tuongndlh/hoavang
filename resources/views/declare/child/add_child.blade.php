							...
							<div class="panel-heading" >
								<div class="form-group row">
									<label class="col-lg-3 control-label">Mã code <span class="text-danger">*</span></label>
										<div class="col-lg-3">
										<input type="text" readonly class="form-control" id="code" name="code" value="{{ $code }}" >
									</div>
								</div>
										...
										<div class="col-sm-3">
											<select style="width: 100%;" class="select form-control"  required="required" name="class" id="class"
											 data-placeholder="Chọn lớp học">
												<option></option>
												@foreach ($class as $item_contact_list)
													<option value="{{ $item_contact_list->id }}">{{ $item_contact_list->name }}</option>
												@endforeach
											</select>
										</div>
								</div>
