    ...
    public function Get_List_Print(Request $request)  {
      ...
      // print_r($id_class);die;
       $query= "select t1.id, t1.name `name_child`, t2.`name` name_class,t1.fee, t3.price,
                   ...
                   WHERE  (t4.month = $pre_month or t4.month is null) $id_class
                   ORDER BY t2.id
                   ";
      $List_data = DB::select($query);
      // print_r($query);die;
       return Datatables::of($List_data)
       ->addColumn('STT', function($List_data){
           return $List_data->id;
        })
      ->addColumn('fee_detail', function($List_data){
           $fee= $List_data->fee;
           ...
           $Cost_NotRB = Cost::whereIn('id',$list_id)->where('rollback',0)->where('is_cost',0)->where('other',0)->get();
           $name_cost__notRB  ='';
           ...
           $Cost_Other = Cost::whereIn('id',$list_id)->where('other',1)->get();
           $name_cost_other  ='';
           $name_cost__other_hide=0;
           if($Cost_Other){
             foreach ($Cost_Other as $item) {
               $name_cost_other .= number_format_drop_zero_decimals($item->price * $item->date_apply,3).'; ';
               $name_cost__other_hide += $item->price * $item->date_apply;
             }
           }
           return $name_cost.$name_cost__notRB.$name_cost_other;
         })
         // Ngày nghỉ
      ->addColumn('day_off', function($List_data){
           $fee= $List_data->fee;
           $list_id = explode(",",str_replace("'","",$fee));
           $Cost = Cost::whereIn('id',$list_id)->where('rollback',1)->get();
           $money_day_off  =0;
           foreach ($Cost as $item) {
             $money_day_off += $item->price * $List_data->dayoff;
              Config::set('money_day_off', $money_day_off);
           }
           return $money_day_off;
        })
        ->addColumn('amount', function($List_data){
            $fee= $List_data->fee;
            $list_id = explode(",",str_replace("'","",$fee));
            // Tính phí ăn sáng, trưa, tối
            $Cost = Cost::whereIn('id',$list_id)->where('rollback',1)->get();
            ...
            $amount_sum = $name_cost_hide+$name_cost__notRB_hide+$name_cost__other_hide+$List_data->price;
            $amount_sub = $money_day_off + $List_data->discount;
            return $amount_sum -$amount_sub;
         })
      ->addColumn('print', function($List_data){
        if($List_data->status == 1)
        {
            $status_print = '<p href="#" class="done_money btn btn-primary" id="'.$List_data->id.'" >
                <i class="fas fa-print"></i> In phiếu  </p>';
        }
        else if($List_data->status == 2)
        {
            $status_print = '<p href="#" class="done_money btn btn-label" id="'.$List_data->id.'" >
                <i class="fas fa-print"></i> In phiếu  </p>';
        }
        else
        {
            $status_print = '<p href="#" class="done_money btn btn-success" id="'.$List_data->id.'" >
                <i class="fas fa-print"></i> In phiếu  </p>';
        }
        return $status_print;
        })
  ...
  public function Print_report_before(Request $request)
     {
          ...
          if($Cost_Other){
            foreach ($Cost_Other as $item) {
              $name_cost__notRB .=
              '<li>'.$item->name.': <b>'.
              number_format_drop_zero_decimals($item->price* $item->date_apply,3).'</b></li>';
              $name_cost__other_hide += $item->price * $item->date_apply;
            }
          }
          ...
          // $money_day_off = Config::get('money_day_off');
          $amount_sum = $name_cost_hide+$name_cost__notRB_hide+$name_cost__other_hide+$List[0]->price;
